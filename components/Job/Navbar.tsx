...
export default function Navbar() {
  ...
  const ThemeToggleButton = () => (
    <motion.button
      ...
      className="relative flex items-center justify-center w-7 h-7 md:w-9 md:h-9 rounded-full bg-slate-100 dark:bg-bg-cold border border-slate-200 dark:border-slate-700/60 hover:bg-slate-200/70 dark:hover:bg-slate-900/30 cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
    ...
    >
      <AnimatePresence mode="wait" initial={false}>
        {theme === "dark" ? (
          ...
          >
            <Moon className="h-4 w-4 md:w-5 md:h-5 text-slate-700 dark:text-slate-300 fill-slate-700/20 dark:fill-slate-300/20" />
          </motion.div>
        ) : (
          <motion.div
            key="sun"
